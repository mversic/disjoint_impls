...
disjoint_impls! {
    ...
    impl<U> Kita for Vec<U> where Option<Vec<U>>: Dispatch<Group = GroupB> {
        const NAME: &'static str = "Blanket B";
    }
    impl<T> Kita for Option<T> where Option<T>: Dispatch<Group = GroupA> {
...
const _: () = {
    ...
    impl<_0> _Kita0<GroupB> for Vec<_0> where Option<Vec<_0>>: Dispatch<Group = GroupB> {
        const NAME: &'static str = "Blanket B";
    }
    impl<_0> _Kita1<GroupA> for Option<_0> where Option<_0>: Dispatch<Group = GroupA> {
        const NAME: &'static str = "Blanket C";
    }
    ...
    impl<_0> Kita for Option<_0> where Option<_0>: Dispatch, Self: _Kita1<<Option<_0> as Dispatch>::Group> {
        const NAME: &'static str = <Self as _Kita1<<Option<_0> as Dispatch>::Group>>::NAME;
    }
};
 */
