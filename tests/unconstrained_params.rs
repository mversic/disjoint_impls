...
const _: () = {
    ...
    impl<T: Dispatch<Group = GroupA>, A: Dispatch<Group = [Z; 1]>, Z> Kita0<GroupA, [A; 1], [Z; 1]>
        for T
    where
        T: Tr<A = [A; 1]>,
    {
        const NAME_šč: &'static str = "1st Blanket AA";
    }
    impl<T: Dispatch<Group = GroupA> + Tr<A = [A; 1]>, A: Dispatch<Group = [Z; 2]>, Z>
        Kita0<GroupA, [A; 1], [Z; 2]> for T
    {
        const NAME_šč: &'static str = "1st Blanket AB";
    }
    impl<T: Dispatch<Group = GroupB> + Tr<A = [A; 2]>, A: Dispatch>
        Kita0<GroupB, [A; 2], <A as Dispatch>::Group> for T
    {
        const NAME_šč: &'static str = "1st Blanket B*";
    }
    pub trait Kita1<_TŠČ0: ?Sized, _TŠČ1: ?Sized, _TŠČ2: ?Sized>: Kita {
        const NAME_šč: &'static str;
    }
    impl<T: Dispatch<Group = GroupA> + Tr<A = [A; 1]>, A: Dispatch<Group = [u16; 1]>>
        Kita1<GroupA, [A; 1], [u16; 1]> for [T; 1]
    {
        const NAME_šč: &'static str = "2nd Blanket AA";
    }
    impl<T: Dispatch<Group = GroupA> + Tr<A = [A; 1]>, A: Dispatch<Group = [u16; 2]>>
        Kita1<GroupA, [A; 1], [u16; 2]> for [T; 1]
    {
        const NAME_šč: &'static str = "2nd Blanket AB";
    }
    impl<T: Dispatch<Group = GroupB> + Tr<A = [A; 2]>, A: Dispatch>
